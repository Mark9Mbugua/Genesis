...
#temporary lists like sessions on the web
products_list = []
product_price = []
product_quantity = []
product_id = []
...
class Application:
    ...
    def ajax(self, *args, **kwargs):
        ...
        result = c.execute(query, (self.get_id, )) #tuple( immutable)
        ...
        # Button change
        self.change_btn = Button(self.left, text="Calculate Change", width=22, height=2, bg='steelblue', command=self.change_func)
    ...
    def add_to_cart(self, *args, **kwargs):
        ...
        if self.quantity_value > int(self.get_stock):
            ...
            for self.p in products_list:
                self.tempname = Label(self.right, text=str(products_list[self.counter]), font=('arial 18 bold'), fg='white', bg='lightblue')
                self.tempname.place(x=0, y=self.y_index)
                labels_list.append(self.tempname)

                self.tempqt = Label(self.right, text=str(product_quantity[self.counter]), font=('arial 18 bold'), fg='white', bg='lightblue')
                self.tempqt.place(x=300, y=self.y_index)
                labels_list.append(self.tempqt)

                self.tempprice = Label(self.right, text=str(product_price[self.counter]), font=('arial 18 bold'), fg='white', bg='lightblue')
                ...
                #total configure
                self.total_l.configure(text="Total: Kshs. " +str(sum(product_price)))
    ...
    def change_func(self, *args, **kwargs):
        ...
        #label change
        self.c_amount = Label(self.left, text="Change: Kshs. " +str(self.to_give), font=('arial 18 bold'), fg='red', bg='white')
        self.c_amount.place(x=0, y=600)

    def generate_bill(self, *args, **kwargs):
        #create the bill before updating the database
        directory = "C:/Users/Mark Mbugua/M.O.B/StoreMS/Invoice/" +str(date) + "/"
        if not os.path.exists(directory):
            os.makedirs(directory)

        # TEMPLATES FOR THE BILL
        company = "\t\t\t\t106 Shopiro Ltd.\n"
        address = "\t\t\t\tPark Road, Ngara\n"
        phone = "\t\t\t\t\t+254712340908\n"
        sample = "\t\t\t\t\tInvoice\n"
        dt = "\t\t\t\t\t" +str(date)
        ...
        for t in products_list:
            f.write("\n\t\t\t\t" +str(r) + "\t\t" + str(products_list[i] + "-------")[:11] +"\t\t" + str(product_quantity[i]) + "\t\t" + str(product_price[i]))
            i += 1
            r += 1
        f.write("\n\n\t\t\tTotal: Kshs. " + str(sum(product_price)))
        f.write("\n\t\t\tThanks for visiting!")
